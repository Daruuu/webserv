...
**Métodos Públicos:**
- `parse()` - Función principal de parsing
- `getServerCount()` - Retorna cantidad de servers

**Métodos Privados Críticos:**
- `ValidateFileExtension()` - Valida extensión `.conf`
- `ValidateFilePermissions()` - Valida permisos de lectura
- `ValidateCurlyBrackets()` - **Recién corregido** - valida balance de `{}`
- `CleanFileConfig()` - Limpia comentarios y espacios
- `RemoveComments()` - Elimina líneas con `#`
- `TrimLine()` - Elimina whitespace
- `NormalizeSpaces()` - Normaliza espacios múltiples
- `extractServerBlocks()` - Extrae bloques `server { }`
...
**Enfoque de testing:**
- Crear archivos `.conf` de prueba en `tests/data/`
