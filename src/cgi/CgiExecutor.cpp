...
CgiProcess* CgiExecutor::executeAsync(const HttpRequest& request, const std::string& script_path,
    ...
    if (pid == 0) {
        ...
        std::map< std::string, std::string > env_map = prepareEnvironment(request, script_path);
...
CgiExecutor::prepareEnvironment(const HttpRequest& request, const std::string& script_path) {
...
char** CgiExecutor::createEnvArray(const std::map< std::string, std::string >& env_map) {
