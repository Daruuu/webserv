 ...
 *
 */
bool HttpParser::splitHeaderLine(const std::string& line, std::string& key, std::string& value) {
...
bool HttpParser::validateHeaders() const {
    ...
    //         return false;
    // }
    if (_request.getVersion() == HTTP_VERSION_1_1 && _request.getHeader("host").empty())
        return false;
