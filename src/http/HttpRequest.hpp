...
class HttpRequest {
  private:
    ...
    std::vector< char >
        _body; // vector para soportar binarios y texto grande (ej: videos, imagenes, etc.)
  public:
    // constructors
    HttpRequest();
    HttpRequest(const HttpRequest& other);
    HttpRequest(const std::string& method, const std::string& version, const HeaderMap& headers,
                const std::string& path, const std::string& query, const std::vector< char >& body);
    ~HttpRequest();
    ...
    void addBody(std::string::const_iterator begin, std::string::const_iterator end);
